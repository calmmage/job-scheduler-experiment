...
def setup_plist_job(script_path, hour=12, minute=0):
    script_path = Path(script_path).resolve()
    if not script_path.exists() or script_path.suffix != ".py":
        raise ValueError("Script must be a valid .py file")

    job_name = f"com.job-scheduler-experiment.{script_path.stem}"
...
if __name__ == "__main__":
    ...
    setup_plist_job(args.script_path, args.hour, args.minute)
